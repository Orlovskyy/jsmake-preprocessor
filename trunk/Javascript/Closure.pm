...
sub minify {
  ...
  my $compilation_level = $args{compilation_level} || SIMPLE_OPTIMIZATIONS;  
  my $js                = $args{input}             || 'var test=true;//this is a test';
     $js =~ s/(\W)/"%" . unpack("H2", $1)/ge if($js!~m{^http://});#urlencode 
  ...
  $req->content("js_code=$js&output_info=$output_info&output_format=$output_format&compilation_level=$compilation_level");
